...
@receiver(
    post_save, sender=CompanyProfile, dispatch_uid="custom_company_profile_validation"
)
def custom_company_profile_validation(sender, instance, **kwargs):
    if (
        instance.address_street
        and instance.address_postal_code
        and instance.address_town
        and instance.name
        and instance.uid
    ):
        company_verifications = CompanyVerification.objects.exclude(
...
def custom_user_profile_validation(sender, instance, **kwargs):
    ...
    if not hasattr(instance, "user_verification"):
        user_verifications = UserVerification.objects.exclude(
            state=VERIFICATION_STATES.CLAIMED.value
        ).filter(
            address_street__iexact=instance.address_street,
            address_town__iexact=instance.address_town,
            ...
            places_of_origin__place_of_origin=instance.place_of_origin,
        )
        # first_name
        ...
        for name in names:
            first_names_lookup = first_names_lookup & (
                Q(first_name__istartswith=name + " ")
                | Q(first_name__iendswith=" " + name)
        ...
        if user_verifications.exists():
            ...
            if (
                instance.owner.user_profiles.filter(
                    user_verification__state=VERIFICATION_STATES.CLAIMED.value
                ).count()
                >= instance.wallet.currency.max_claims
            ):
