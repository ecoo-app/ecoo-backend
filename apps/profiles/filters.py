    ...
    def queryset(self, request, queryset):
        if self.value() == "0":
            ...
            return queryset.filter(
                sms_pin_verifications__state=VERIFICATION_STATES.PENDING.value
            )
        elif self.value() == "2":
            return queryset.filter(
                Q(user_verification__state=VERIFICATION_STATES.CLAIMED.value)
                | Q(sms_pin_verifications__state=VERIFICATION_STATES.CLAIMED.value)
            )
        elif self.value() == "3":
            return queryset.filter(
                user_verification__state=VERIFICATION_STATES.MAX_CLAIMS.value
            )
        return queryset.all()
...
class CompanyVerificationLevelFilter(UserVerificationLevelFilter):
    ...
    def queryset(self, request, queryset):
        if self.value() == "0":
            return queryset.exclude(company_verification__isnull=False).exclude(
                address_pin_verification__isnull=False
            )
        elif self.value() == "1":
