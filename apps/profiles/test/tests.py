...
class ProfileApiTest(EcouponTestCaseMixin, APITestCase):
    def test_new_verification_flow_without_sms_consumer(self):
        ...
        response = self.client.post(
            "/api/wallet/wallet/",
            {"public_key": key.public_key(), "currency": currency.uuid},
            format="json",
        )
        wallet = Wallet.objects.get(public_key=key.public_key())
        ...
        # generate "verification transaction"
        paper_wallet = PaperWallet.objects.create(
            currency=currency,
            private_key=key2.secret_key(),
            wallet_id=PaperWallet.generate_wallet_id(),
            public_key=key2.public_key(),
            category=WALLET_CATEGORIES.CONSUMER.value,
        )
        Transaction.objects.create(to_wallet=paper_wallet, amount=20)

        tx = MetaTransaction(
            from_wallet=paper_wallet,
            to_wallet=wallet,
            nonce=1,
            amount=paper_wallet.balance,
        )

        signature = key2.sign(
            pack_meta_transaction(tx.to_meta_transaction_dictionary())
        )
    ...
    def test_new_verification_flow_with_sms_consumer(self):
        ...
        sms_verification = SMSPinVerification.objects.create(
            user_profile=user_profile, pin="1234"
        )
    ...
    def test_new_verification_flow_without_sms_company(self):
        currency = Currency.objects.create(
            token_id=0,
            name="no sms",
            symbol="tez",
        ...
        # Transaction.objects.create(to_wallet=paper_wallet, amount=0)
    ...
    def test_new_verification_flow_without_sms_company_not_valid(self):
        currency = Currency.objects.create(
            token_id=0,
            name="no sms",
            symbol="tez",
    ...
    def test_new_verification_flow_with_sms_company(self):
        currency = Currency.objects.create(
            token_id=0,
            name="no sms",
            symbol="tez",
        ...
        # sms verification
        sms_verification = SMSPinVerification.objects.create(
            company_profile=profile, pin="1234"
        )
        sms_verification.state = VERIFICATION_STATES.CLAIMED.value
        sms_verification.save()
    ...
    def test_user_profile_verification_flow(self):
        ...
        response = self.client.post("/api/profiles/user_profiles/", data, format="json")

        self.assertEqual(response.status_code, status.HTTP_401_UNAUTHORIZED)

        self.assertEqual(user_verification.state, VERIFICATION_STATES.OPEN.value)
        self.assertEqual(self.wallet_1.balance, old_balance)
        ...
        user_verification = UserVerification.objects.get(pk=user_verification.pk)
        self.assertEqual(user_verification.state, VERIFICATION_STATES.PENDING.value)
        self.assertEqual(self.wallet_1.balance, old_balance)

        user_profile = UserProfile.objects.get(pk=response.data["uuid"])
        sms_pin_verification = user_profile.sms_pin_verification

        self.assertEqual(sms_pin_verification.state, VERIFICATION_STATES.PENDING.value)

        self.client.force_authenticate(user=self.user_2)

        # TODO: shouldn't test for those endpoints be inside apps.verification.tests ?
        response = self.client.post(
            "/api/verification/resend_user_profile_pin/{}".format(user_profile.pk),
            data,
            format="json",
        )
        ...
        response = self.client.post(
            "/api/verification/verify_user_profile_pin/{}".format(user_profile.pk),
            {"pin": "WRONG"},
            format="json",
        )
        self.assertEqual(response.status_code, status.HTTP_422_UNPROCESSABLE_ENTITY)
        self.assertEqual(
            ...
            "You are retrying too fast, please wait for 14 seconds",
        )
        ...
        self.assertEqual(user_verification.state, VERIFICATION_STATES.CLAIMED.value)

        sms_pin_verification.refresh_from_db()
        self.assertEqual(sms_pin_verification.state, VERIFICATION_STATES.CLAIMED.value)

        self.wallet_1.refresh_from_db()
        self.assertEqual(self.wallet_1.balance, old_balance + 10)
    ...
    def test_company_verification_ok(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
        self.assertEqual(
            response.data["verification_stage"],
        ...
        response = self.client.post(
            "/api/verification/verify_company_profile_pin/{}".format(
                company_profile.pk
            ),
            {"pin": pin},
            format="json",
        ...
        self.assertEqual(response.status_code, status.HTTP_204_NO_CONTENT)
        self.assertEqual(
            company_profile.verification_stage(),
    ...
    def test_company_profile_verification_flow(self):
        ...
        self.assertEqual(company_verification.state, VERIFICATION_STATES.OPEN.value)
        ...
        self.assertEqual(company_verification.state, VERIFICATION_STATES.PENDING.value)
        ...
        self.assertEqual(
            address_pin_verification.state, VERIFICATION_STATES.PENDING.value
        )
        ...
        self.assertEqual(company_verification.state, VERIFICATION_STATES.CLAIMED.value)
        address_pin_verification = AddressPinVerification.objects.get(
            pk=company_profile.address_pin_verification.pk
        )
        self.assertEqual(
            address_pin_verification.state, VERIFICATION_STATES.CLAIMED.value
        )
    ...
    def test_user_profile_destroy(self):
        ...
        response = self.client.delete(
            "/api/profiles/user_profiles/{}/".format(user_2_profile_uuid),
            data,
            format="json",
        )
        ...
        response = self.client.delete(
            "/api/profiles/user_profiles/{}/".format(user_1_profile_uuid),
            data,
            format="json",
        )
        self.assertEqual(response.status_code, status.HTTP_204_NO_CONTENT)
        self.assertEqual(UserProfile.objects.all().count(), user_profile_count - 1)
    ...
    def test_company_profile_verification_flow_2(self):
        ...
        self.client.force_authenticate(user=self.user_2)
        response = self.client.delete(
            "/api/profiles/company_profiles/{}/".format(user_1_profile_uuid),
            data,
            format="json",
        )
        self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND)
    ...
    def test_to_many_verifications_company(self):
        ...
        for i in range(5):
            company_verification = CompanyVerification.objects.create(
                name="Papers AG" + str(i), uid="12-3-4-3"
            )
            ...
            self.assertEqual(
                company_verification.state, VERIFICATION_STATES.CLAIMED.value
            )

        company_verification = CompanyVerification.objects.create(
            name="Papers AG", uid="12-3-4-3"
        )
    ...
    def test_to_many_verifications_user(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_204_NO_CONTENT)
        user_verification = UserVerification.objects.get(pk=user_verification.pk)
        self.assertNotEqual(user_verification.state, VERIFICATION_STATES.CLAIMED.value)
...
class ProfileLookupTest(APITestCase):
    ...
    def test_user_complete(self):
        ...
        user_verification.refresh_from_db()
        self.assertEquals(user_verification.state, VERIFICATION_STATES.PENDING.value)
    ...
    def test_first_names3(self):
        ...
        UserProfile.objects.create(
            owner=self.user,
            first_name="Lionel Alessandro Markus",
            last_name="De Carli",
            address_street="Sonnmattstr. 121",
            address_postal_code="5242",
            address_town="Birr",
        ...
        user_verification.refresh_from_db()
        self.assertEquals(user_verification.state, VERIFICATION_STATES.OPEN.value)
    ...
    def test_company_lookup(self):
        ...
        self.assertEquals(company_verification.state, VERIFICATION_STATES.PENDING.value)
