...
def send_postcard(text='', title='', firstname='', lastname='', company='', street='', house_nr='', zip='', city='', country='Switzerland', po_box='', additional_address_info=''):
    ...
    client = BackendApplicationClient(client_id=POST_API_CONFIG['client_id'], scope=POST_API_CONFIG['scope'])
    oauth = OAuth2Session(client=client)
    token = oauth.fetch_token(token_url=POST_API_CONFIG['token_url'], client_id=POST_API_CONFIG['client_id'], client_secret=POST_API_CONFIG['client_secret'])

    url = POST_API_CONFIG['base_url'] + 'v1/postcards?campaignKey=' + POST_API_CONFIG['campaign_key']
