...
from apps.verification.utils import send_postcard, send_sms
...
@receiver(
    pre_save,
    sender=SMSPinVerification,
    dispatch_uid="custom_sms_pin_verification_validation",
)
def custom_sms_pin_verification_validation(sender, instance, **kwargs):
    if not instance.pin:
        instance.pin = "".join(system_random.choice(string.digits) for x in range(6))
        try:
...
@receiver(
    pre_save,
    sender=AddressPinVerification,
    dispatch_uid="custom_address_pin_verification_validation",
)
