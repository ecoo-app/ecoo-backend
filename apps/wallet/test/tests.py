...
class WalletTestCase(TestCase):
    def setUp(self):
        pre_save.disconnect(
            custom_meta_transaction_validation,
            sender=MetaTransaction,
            dispatch_uid="custom_meta_transaction_validation",
        )
    ...
    def test_address_calculation(self):
        ...
        self.assertEqual(wallet.nonce, 0)
    ...
    def test_nonce_calculation(self):
        ...
        meta_transaction1 = MetaTransaction(
            from_wallet=wallet1, to_wallet=wallet2, amount=10, nonce=1
        )
        signature1 = key1.sign(
            pack_meta_transaction(meta_transaction1.to_meta_transaction_dictionary())
        )
        meta_transaction1.signature = signature1
        meta_transaction1.save()
        self.assertEqual(wallet1.nonce, 1)
        self.assertEqual(wallet2.nonce, 0)

        meta_transaction2 = MetaTransaction(
            from_wallet=wallet2, to_wallet=wallet1, amount=1, nonce=1
        )
        signature2 = key2.sign(
            pack_meta_transaction(meta_transaction2.to_meta_transaction_dictionary())
        )
        meta_transaction2.signature = signature2
        meta_transaction2.save()

        self.assertEqual(wallet1.nonce, 1)
        self.assertEqual(wallet2.nonce, 1)

        meta_transaction3 = MetaTransaction(
            from_wallet=wallet1, to_wallet=wallet2, amount=1, nonce=2
        )
        signature3 = key1.sign(
            pack_meta_transaction(meta_transaction3.to_meta_transaction_dictionary())
        )
        meta_transaction3.signature = signature3
        meta_transaction3.save()
        self.assertEqual(wallet1.nonce, 2)
        self.assertEqual(wallet2.nonce, 1)
    ...
    def test_balance_calculation(self):
        ...
        self.assertEqual(wallet1.balance, 91)
        self.assertEqual(wallet2.balance, 9)
...
class PaperWalletTestCase(TestCase):
    def setUp(self):
...
class ComplexTransactionFlowsTestCase(TestCase):
    def setUp(self):
        ...
        self.wallet1 = Wallet.objects.create(
            wallet_id=Wallet.generate_wallet_id(),
            public_key=self.key1.public_key(),
            currency=self.currency,
            state=WALLET_STATES.VERIFIED.value,
        )
        self.wallet2 = Wallet.objects.create(
            wallet_id=Wallet.generate_wallet_id(),
            public_key=self.key2.public_key(),
            currency=self.currency,
            state=WALLET_STATES.VERIFIED.value,
        )
    ...
    def test_recovery_flow_with_history(self):
        ...
        meta_transaction1 = MetaTransaction(
            from_wallet=self.wallet1, to_wallet=self.wallet2, amount=10, nonce=1
        )
        signature1 = self.key1.sign(
            pack_meta_transaction(meta_transaction1.to_meta_transaction_dictionary())
        )
        meta_transaction1.signature = signature1
        meta_transaction1.save()
        ...
        meta_transaction2 = MetaTransaction(
            from_wallet=self.wallet1, to_wallet=self.wallet2, amount=10, nonce=2
        )
        signature2 = self.key1.sign(
            pack_meta_transaction(meta_transaction2.to_meta_transaction_dictionary())
        )
        meta_transaction2.signature = signature2
        meta_transaction2.save()
        ...
        wallet_public_key_transfer_request = (
            WalletPublicKeyTransferRequest.objects.create(
                wallet=self.wallet1,
                old_public_key=self.wallet1.public_key,
                new_public_key=new_key1.public_key(),
            )
        )
        ...
        meta_transaction2 = MetaTransaction(
            from_wallet=self.wallet1, to_wallet=self.wallet2, amount=10, nonce=3
        )
        signature2 = new_key1.sign(
            pack_meta_transaction(meta_transaction2.to_meta_transaction_dictionary())
        )
        meta_transaction2.signature = signature2
        # the nonce must reset because it's a new pubkey
        with self.assertRaises(ValidationError):
            meta_transaction2.save()
...
class BlockchainSyncTestCase(TestCase):
    def setUp(self):
        ...
        self.wallet1 = Wallet.objects.create(
            wallet_id=Wallet.generate_wallet_id(),
            public_key=self.pytezos_client.key.public_key(),
            currency=self.currency,
            state=WALLET_STATES.VERIFIED.value,
        )
        self.wallet2 = Wallet.objects.create(
    ...
    def test_wallet_recover_transfer(self):
        ...
        WalletPublicKeyTransferRequest.objects.create(
            wallet=self.wallet1,
            old_public_key=self.wallet1.public_key,
            new_public_key="edpkvThfdv8Efh1MuqSTUk5EnUFCTjqN6kXDCNXpQ8udN3cKRhNDr2",
        )
        ...
        start = time.time()
        self.assertEquals(True, sync_to_blockchain(is_dry_run=True))
        end = time.time()
        print(end - start)
    ...
    def test_complex_sync(self):
        for i in range(40):
            ...
            WalletPublicKeyTransferRequest.objects.create(
                wallet=user_wallet,
                old_public_key=user_wallet.public_key,
                new_public_key=public_key,
            )

        self.assertEquals(
            81, Transaction.objects.filter(state=TRANSACTION_STATES.OPEN.value).count()
        )  # 50 meta, 50 funding, 1 mint
        start = time.time()
        self.assertEquals(True, sync_to_blockchain(is_dry_run=False))
        end = time.time()
        self.assertEquals(
            81, Transaction.objects.filter(state=TRANSACTION_STATES.DONE.value).count()
        )
        print(end - start)
