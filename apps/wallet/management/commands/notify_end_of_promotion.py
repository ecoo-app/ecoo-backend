...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            ...
            nargs="+",
            default=[15, 8, 3],
            help="a amount of days before to notify the deadline",
        )
                ...
                for category, msg in mapping:
                    users_to_notify = (
                        Wallet.objects.filter(
                            currency=currency, balance__gt=0, category=category
                        )
                        .values_list("owner")
                        .distinct()
                    )
                    ...
                    self.stdout.write(
                        f"Currency: {currency.name} Notifying {len(devices)} potential devices"
                    )
                    devices.send_message(
                        title=settings.PUSH_NOTIFICATION_TITLE,
                        body=msg.replace("CURRENCY_NAME", currency.name).replace(
                            "XXX", days
                        ),
                    )
